 ...
 * @author 高贵血迹
 */global $_W, $_GPC;
   $operation = in_array ( $_GPC ['op'], array ('default','export','input_cj','input_tea','input_teabj','input_teapf','input_stu','input_stupf','input_card','input_cardschool','input_kc','input_ks','input_scpy') ) ? $_GPC ['op'] : 'default';
	...
	if($operation == 'input_cj'){ //导入成绩
		...
		//年级处理
		$xueqi = pdo_fetch("SELECT parentid FROM " . tablename($this->table_classify) . " WHERE sid=:sid  And schoolid=:schoolid ", array(':sid' => $banji['sid'],  ':schoolid'=>$schoolid));
		$strss = '';
		foreach($strs as $row){
			$strss .= $row.',';
		}
	...
	if($operation == 'input_stu'){ //学生
		...
		//班级处理
		$banji = pdo_fetch("SELECT sid FROM " . tablename($this->table_classify) . " WHERE sname=:sname And schoolid=:schoolid And type=:type", array(':sname' => trim($strs[9]), ':schoolid'=> $schoolid, ':type'=> 'theclass'));
		$strss = '';
		foreach($strs as $row){
			$strss .= $row.',';
		}
		$result ['tips'] = '';
		if(empty($xueqi) || empty($banji)){
			if(empty($xueqi)){
				$result ['tips'] .= '|无此年级-'.trim($strs[8]).'|';
			}
			if(empty($banji)){
