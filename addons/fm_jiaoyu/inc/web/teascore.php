...
if($operation == 'post'){
    ...
    if(!empty($id)){
		...
		if($item['fromtid'] != 'founder' && $item['fromtid'] !='owner' ){
			...
			$fromfz = pdo_fetch("SELECT name FROM " . tablename($this->table_classify) . " where sid = :sid ", array(':sid' => $fromteacher['fz_id']));
		}else{
		...
		foreach($data as $strs) {
			...
			if(empty($tid)){
				...
				if($_GPC['fromtid'] == 'founder' || $_GPC['fromtid'] == 'owner' ){
					...
					$fromtea = pdo_fetch("SELECT fz_id FROM " . tablename($this->table_teachers) . " WHERE tid=:tid AND weid=:weid And schoolid=:schoolid  ", array(':tid' => $_GPC['fromtid'], ':weid' => $_W['uniacid'], ':schoolid'=> $schoolid));
