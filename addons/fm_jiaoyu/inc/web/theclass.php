        ...
        if(!empty($sid)){	
			...
			if(!empty($_GPC['new'])){
				foreach($_GPC['new'] as $key => $name){
					...
					if(empty($name)){
						$this->imessage('抱歉，班级名称不能为空！', referer(), 'error');
					}
					if(empty($_GPC['parentid_new'][$key])){
						$this->imessage('抱歉，请选择所属年级！', referer(), 'error');
					}					
					$data = array(
						'weid'     => $weid,
			...
			if(!empty($_GPC['new'])){
				...
				$this->imessage('新增班级成功！', $this->createWebUrl('theclass', array('op' => 'display', 'schoolid' => $schoolid)), 'success');	
			}					
		}
    }
