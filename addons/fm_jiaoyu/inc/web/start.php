...
// 如果是星河大学，则将校园概括改为对所有学校的统计页面
if($operation == 'display' && $schoolid == 17){
    $todayTime = mktime(0,0,0,date("m"),date("d"),date("Y"));
    $tomorrowTime = $todayTime + 3600*24;
    $schools = pdo_fetchall("SELECT id,title FROM ". tablename($this->table_index) . " order by title ");
    $len = sizeof($schools);
    $i=0;
    foreach ($schools as $key=>$value){
        if(strpos($value['title'], "文教办") !== false || strpos($value['title'], "公安") !== false)
            continue;
        $schoolCheckLog[$key]['id'] = $value['id'];
        $schoolCheckLog[$key]['title'] = $value['title'];
        $check = pdo_fetch("SELECT createtime FROM " . tablename($this->table_checklog) . " WHERE schoolid = '{$value['id']}' ORDER by id DESC " );
        $today = strtotime(date("Y-m-d", time()));
        if($check['createtime'] < $today){
            $schoolCheckLog[$key]['lastcheck'] = "<font color=\"red\">" . date("Y-m-d H:i:s", $check['createtime']) . "</font>";
        }else
            $schoolCheckLog[$key]['lastcheck'] = date("Y-m-d H:i:s", $check['createtime']);

        $logs = pdo_fetchall("SELECT createtime, msg FROM " . tablename($this->table_log) . " WHERE schoolid = '{$value['id']}' AND createtime > $todayTime AND createtime < $tomorrowTime " );
        foreach ($logs as $log)
          $schoolCheckLog[$key]['log'] .= date("Y-m-d H:i:s", $log['createtime'])."    ".$log['msg'] . "<br>";

        $suc = 0;
        $lastid = 0;
        /*if($value['id'] == 9){
            $handle = fopen('./2019-4-2_Log.log', 'r');
            while(!feof($handle)){
                $line = fgets($handle, 1024);
                if(strlen($line) > 10){
                    $signId = substr($line, strpos($line, "signId=") + 7, 10);
                    if(is_numeric($signId)){
                        if($signId == $lastid)
                            continue;
                        $res = pdo_fetch("SELECT * FROM " . tablename($this->table_checklog) . " WHERE cardid = '{$signId}' AND createtime > $todayTime AND createtime < $tomorrowTime ");
                        if(empty($res)){
                            $schoolCheckLog[$key]['log'] .= "<font color=\"red\">"."打卡失败的卡号：".$signId. "</font>"."<br>";
                        }
                        else{
                            $sid = pdo_fetch("SELECT sid FROM " . tablename($this->table_idcard) . " WHERE idcard = '{$signId}' " );
                            if(($res['leixing'] == 1 || $res['leixing'] == 3) && $sid['sid'] != 0) {
                                $schoolCheckLog[$key]['log'] .= "<font color=\"red\">"."打卡成功的卡号：".$signId. "</font>"."<br>";
                                $suc++;
                            }
                        }
                        $lastid = $signId;
            ...
            $schoolCheckLog[$key]['log'] .= "打卡成功数：".$suc."<br>";
            fclose($handle);
        }*/
    }
