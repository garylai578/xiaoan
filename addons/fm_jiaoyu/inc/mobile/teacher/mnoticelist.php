 ...
 */
		...
		if($thistime){
		    ...
		    $leave1 = pdo_fetchall("SELECT id,title,tname,createtime,type,tid,content,usertype,userdatas FROM " . tablename($this->table_notice) . " where weid = '{$weid}' And schoolid = '{$schoolid}' And tid = '{$tid_global}' $condition $condition2 ORDER BY createtime DESC LIMIT 0,5 ");
