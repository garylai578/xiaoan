					 ...
					 if($school['bjqstyle'] == 'old'){
						 $photoUrls = explode ( ',', $_GPC ['photoUrls'] );
							...
							foreach($photoUrls as $key => $v){
								if(!empty($v)) {
									$url = 'https://file.api.weixin.qq.com/cgi-bin/media/get?access_token='.$token2.'&media_id='.$v;
									$pic_data = ihttp_request($url);
									$path = "images/bjq/img/";
									$picurl = $path.random(30) .".jpg";
									file_write($picurl,$pic_data['content']);
	...
	if ($operation == 'sfabu') {
		...
		if (! $_GPC ['schoolid'] || ! $_GPC ['weid']) {
			...
			if ($user['status'] == 1) {
				...
				$video = '';
				$school = pdo_fetch("SELECT isopen,txid,txms,bjqstyle,savevideoto FROM " . tablename($this->table_index) . " WHERE :weid = weid And :id = id", array(':weid' => $_GPC['weid'], ':id' => $_GPC['schoolid']));
				if(!empty($_GPC['videoMediaId'])){
					$msgtype = 3;//视频
					$HttpUrl="vod.api.qcloud.com";
					$HttpMethod="GET"; 
