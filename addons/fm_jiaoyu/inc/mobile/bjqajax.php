	...
	if ($operation == 'sfabu') {
		...
		if (! $_GPC ['schoolid'] || ! $_GPC ['weid']) {
			...
			if ($user['status'] == 1) {
				...
				$video = '';
				$school = pdo_fetch("SELECT isopen,txid,txms,bjqstyle,savevideoto,sh_teacherids FROM " . tablename($this->table_index) . " WHERE :weid = weid And :id = id", array(':weid' => $_GPC['weid'], ':id' => $_GPC['schoolid']));
				if(!empty($_GPC['videoMediaId'])){
					$msgtype = 3;//视频
					$HttpUrl="vod.api.qcloud.com";
					$HttpMethod="GET"; 
