 ...
 * @author 高贵血迹
 */global $_W, $_GPC;
   $operation = in_array ( $_GPC ['op'], array ('default','getbjlist','bdxs','skcsign','tkcsign','xskcqdqr','xskcbq','qrjsqd','txsk','signup','newstu','get_kslist','delsign_one','getxgtemplte','sqingjia','kcpingjia','txkcpj','deletetempstu') ) ? $_GPC ['op'] : 'default';
	...
	if ($operation == 'skcsign') {
		...
		if (! $_GPC ['schoolid'] || ! $_GPC ['weid']) {
			...
			$checkkc = pdo_fetch("select id,tid,maintid,OldOrNew,FirstNum FROM ".tablename($this->table_tcourse)." WHERE weid='{$_GPC['weid']}' And schoolid='{$_GPC['schoolid']}' And  id = '{$_GPC['kcid']}'");
			if(empty($checkkc)){
				die ( json_encode ( array (
                    'result' => false,
                    'msg' => '该课程不存在！' 
