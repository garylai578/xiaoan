...
<div id="titlebar" class="header mainColor">
	...
	<div class="m"><a><span style="font-size: 18px">校园通知动态群发</span></a></div>
...
<body>
<div class="All">       
	<div class="top_head_blank" id="titlebar_bg"></div>
	...
	<?php  if($teachers['status'] == 2) { ?>
	 <div class="F_div qx_00202" style="margin-bottom:95px;" onclick="showCheckBox();">
        <div class="F_div_text">发布</div>
    </div>
    <?php  } ?>
<div class="clear"></div>
<div class="clear"></div>
...
// 底部加载更多
	new Scroll_load({
		...
		"ajax_url": "<?php  echo $this->createMobileUrl('mnoticelist', array('schoolid' => $schoolid, 'bj_id' => $bj_id), true)?>",
		...
		"after_ajax": function () {
			...
			strConvertHtml();
			<?php  if(!(IsHasQx($tid_global,2000202,2,$schoolid))) { ?>
				$(".qx_00202").hide();
			<?php  } ?>
			<?php  if(!(IsHasQx($tid_global,2000203,2,$schoolid))) { ?>
				$(".qx_00203").hide();
			<?php  } ?>
		}
	}).load_init();
	   ...
	   strConvertHtml();
		...
		$('body').on('click',  '.main', function(e) {
			if (!$(e.target).parent().is('.notifyImgItem')) {
				...
				window.location.href = "<?php  echo $this->createMobileUrl('mnotice', array('schoolid' => $schoolid))?>"+ '&id=' + notifyUid;
			} 
		});
