	...
	<div class="listContent">
		...
		<li class="main" time="<?php  echo $v['createtime'];?>" id="<?php  echo $v['id'];?>" style="display: block;">
			<div class="tongzhi">
				...
				<!-- <div class="btnEditBox" onclick="itemEdit(<?php  echo $v['id'];?>);return false;"> -->
					<!-- <img src="<?php echo OSSURL;?>public/mobile/img/btn_edit_01.png" class="img-responsive"> -->
				<!-- </div> -->
				<div class="btnDeleteBox qx_00103" style="top: 30%;right: 60px;" onclick="del_notice(<?php  echo $v['id'];?>);return false;">
...
<div class="clear"></div>	
			...
			<?php  $bjlistsss = array();?>
				<?php  if(is_array($bjlists)) { foreach($bjlists as $row) { ?>
					...
					<?php  $bjlistsss[] = $row['bj_id'];?>
						<li onclick="isSelect(<?php  echo $row['sid'];?>);" <?php  if($bj_id == $row['sid']) { ?>class="act"<?php  } ?>><div style="margin-left: -47px;font-size: 13px;"><?php  echo $row['sname'];?><?php  if($row['info']) { ?><span style="color:red;"><?php  echo $row['info'];?></span><?php  } ?></div></li>
					<?php  } ?>
				<?php  } } ?>
			</ul>
		</div>
    ...
    <div class="F_div qx_00102" onclick="addnote();">
        <div class="F_div_text">写通知</div>
    </div>
...
<script type="text/javascript">
	...
	$(function ($) {
		...
		<?php  if(!(IsHasQx($tid_global,2000102,2,$schoolid))) { ?>
			$(".qx_00102").hide();
		<?php  } ?>
		<?php  if(!(IsHasQx($tid_global,2000103,2,$schoolid))) { ?>
			$(".qx_00103").hide();
		<?php  } ?>
	...
	function isSelect(bjid){
		jTips("数据加载中！···");
		location.href = "<?php  echo $this->createMobileUrl('noticelist', array('schoolid' => $schoolid), true)?>"+ '&bj_id=' + bjid;
	}
	...
	function addnote(){
		location.href = "<?php  echo $this->createMobileUrl('fabu', array('schoolid' => $schoolid), true)?>";
	}
...
// 底部加载更多
	new Scroll_load({
		...
		"ajax_url": "<?php  echo $this->createMobileUrl('noticelist', array('schoolid' => $schoolid, 'bj_id' => $bj_id), true)?>",
	   ...
	   strConvertHtml();
		...
		$('body').on('click',  '.main', function(e) {
			if (!$(e.target).parent().is('.notifyImgItem')) {
				...
				window.location.href = "<?php  echo $this->createMobileUrl('notice', array('schoolid' => $schoolid))?>"+ '&id=' + notifyUid;
			} 
		});
