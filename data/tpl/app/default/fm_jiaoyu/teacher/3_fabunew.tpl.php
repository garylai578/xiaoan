...
<div id="titlebar" class="header mainColor">
	...
	<div class="m">
		<span style="font-size: 18px">创建班级通知</span>   
	</div>
</div>    
...
<div class="feedback_box">
    ...
    <div class="feedback_title_box">
        <div class="feedback_title feedback_title_teacher" id="feedback_title" select_id=''>请选择</div>
	...
	<div class="feedback_title_box">
		<div class="textBox">
			<input style="border:none" id="title" type="text" placeholder="文章标题...">
        </div>
	</div>	
    <div class="blank"></div>
    ...
    <div class="weui_cell_ft">
        <span>同时发表至班级圈&nbsp;</span><input id="is_personal" class="weui_switch" type="checkbox" value="私密"/>
    </div>
    <div class="blank"></div>
...
<script type="text/javascript">
		...
		var choose_img_params = {
			...
			wxsdkSendData: function (imgServerid, videoServerid, videoTime, media_receiveid) {
				if (submit_wxsdkSendData) {
					...
					var url = "<?php  echo $this->createMobileUrl('dongtaiajax',array('op'=>'fabu'))?>";
	...
	function upload_pro(page,noticeidarr,bjarr,stuarr,total, self) {
		...
		$.ajax({
			url: "<?php  echo $this->createMobileUrl('dongtaiajax', array('schoolid' => $schoolid,'op' => 'notpro'))?>",
			data: prodata,
			type: "POST",
			dataType: "json",
			success: function (result) {
				...
				if (result.status == 2) {
					...
					location.href = "<?php  echo $this->createMobileUrl('noticelist', array('schoolid' => $schoolid))?>";
				}				
			},
			error: function () {
				//提交后 隐藏加载层
