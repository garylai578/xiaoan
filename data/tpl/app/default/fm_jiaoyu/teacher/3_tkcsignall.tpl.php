...
.slide_left_menu_ul li {height: 50px;line-height: 50px;/*border-bottom: 1px solid #ccc;*/font-size: 16px;width: 100%;box-sizing: border-box;padding: 0 10px 0 20px;overflow: hidden;
position: relative;}
...
<div class="header mainColor">
	...
	<div class="m"><a><span style="font-size: 18px">教师授课信息列表</span></a></div>
	<div class="r">
...
<div class="All">       
	...
	<div class="listContent">
		<?php  if(is_array($allteacher)) { foreach($allteacher as $v) { ?>
...
<div class="clear"></div>	
	<div class="slide_left_menu_bg">
		<div class="slide_left_menu">
			...
			<ul class="slide_left_menu_ul">
				<li onclick="isSelect(0);" <?php  if($nj_id == 0) { ?>class="act"<?php  } ?>><div style="text-align: right;padding-right:20px">不限年级</div></li>	
				<?php  if(is_array($allnj)) { foreach($allnj as $row) { ?>
				<li onclick="isSelect(<?php  echo $row['sid'];?>);" <?php  if($nj_id == $row['sid']) { ?>class="act"<?php  } ?>><div style="text-align: right;padding-right:20px"><?php  echo $row['sname'];?></div></li>
				<?php  } } ?>		
			</ul>
		</div>
	</div>	
    ...
    <div class="F_div qx_01502" onclick="signconfirm();">
        <div class="F_div_text" style="margin: 10px 0 0 0px;">签到</br>确认</div>
    </div>
</div>
...
<script type="text/javascript">
	...
	$(function ($) {
		...
		<?php  if(!IsHasQx($tid_global,2001502,2,$schoolid) ) { ?>
			$(".qx_01502").hide();
		<?php  } ?>
	...
	function isSelect(njid){
		jTips("数据加载中！···");
		location.href = "<?php  echo $this->createMobileUrl('tkcsignall', array('schoolid' => $schoolid), true)?>"+ '&nj_id=' + njid;
	}
	...
	function signconfirm(){
		location.href = "<?php  echo $this->createMobileUrl('tqrjsqd', array('schoolid' => $schoolid), true)?>";
	}
...
// 底部加载更多
	new Scroll_load({
		...
		"ajax_url": "<?php  echo $this->createMobileUrl('tkcsignall', array('schoolid' => $schoolid, 'nj_id' => $nj_id), true)?>",
	...
	$(function () {
		...
		$('body').on('click',  '.main', function(e) {
			if (!$(e.target).parent().is('.notifyImgItem')) {
				...
				window.location.href = "<?php  echo $this->createMobileUrl('tkcsigndetail', array('schoolid' => $schoolid))?>"+ '&id=' + notifyUid;
			} 
		});
	});
</script>
