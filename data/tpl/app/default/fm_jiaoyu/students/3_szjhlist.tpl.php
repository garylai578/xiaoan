...
.wkplan_list li.active:after{content: '';width: 0.6rem;height:0.32rem;background: url(<?php echo OSSURL;?>public/mobile/img/weekplan_now.png) no-repeat;background-size: 100% 100%;background-position: center center;position: absolute;left:-0.3rem;top:50%;margin-top: -0.2rem;z-index: 1;text-align: center;}
...
<div class="All">  
	<div style="height:50px;" id="titlebar_bg"></div>
	...
	<div class="wkplan_list_wrap">
		...
		<?php  if(is_array($weekplanlist)) { foreach($weekplanlist as $row) { ?>
			<?php  if($row['start']<TIMESTAMP && $row['end']>TIMESTAMP) { ?><li class ="active"><?php  } else { ?><li><?php  } ?>
				<a href="<?php  echo $this->createMobileUrl('szjh', array('schoolid' => $schoolid,'id' => $row['planuid'],'bj_id' => $row['bj_id'],'op' => $row['type']), true)?>">
				(<?php  echo date('Y年', $row['start'])?>) <?php  echo date('m月d日', $row['start'])?>-<?php  echo date('m月d日', $row['end'])?>
				<?php  if($row['type'] == 'word') { ?><span class="wd" style="color:#fff;">计划表</span><?php  } else { ?><span class="wd" style="color:#fff;">图片版</span><?php  } ?>
				</a>
			</li>                              
		<?php  } } ?>
		</ul>
