...
var divids = mfid + 40;
	$('#mof').click(function(){
		...
		var htmls1ss = '<div id="iconeditor'+divids+'" class="editor" style="top:200px">'+
					...
					'										<div class="input-group">'+
					'											<input type="text" name="bzcolor['+divids+']" value="" class="form-control ng-pristine ng-valid ng-touched">'+
					'									<span class="help-block">请在其他生效魔方按钮中选择颜色后复制至此</span>'+
					'										</div>'+
					'									</div>'+
					'								</div>'+
					'							</div>'+
					...
					'							<div class="col-xs-9">'+
					'								<input type="text" id="mfbzs'+divids+'" name="mfbzs['+divids+']" onkeyup="SwapTxt2('+divids+')" placeholder="按钮描述" value="底部小字" class="form-control ng-pristine ng-untouched ng-valid">'+
					'							</div>'+
					'						</div>'+									
					...
					'									<span class="input-group-btn">'+
					'										<button class="btn btn-default" type="button" data_id="mofang'+divids+'" onclick="showImageDialogmfs(this);">选择图片</button>'+
					'									</span>'+
					'								</div>'+
