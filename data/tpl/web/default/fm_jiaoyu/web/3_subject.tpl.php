...
<ul class="nav nav-tabs">
    <li class="qx_edit <?php  if($operation == 'post') { ?>active<?php  } ?>"><a href="<?php  echo $this->createWebUrl('subject', array('op' => 'post', 'schoolid' => $schoolid))?>">添加科目</a></li>
    <li <?php  if($operation == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('subject', array('op' => 'display', 'schoolid' => $schoolid))?>">科目管理</a></li>
</ul>
				...
				<?php  if(!empty($sid)) { ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-2 col-lg-2">
							<input type="text" name="ssort" placeholder="排序" class="form-control" value="<?php  echo $subject['ssort'];?>" />
						</div>
						
						<label class="col-sm-2" style="width:8%">科目名称</label>
						<div class="col-sm-2 col-lg-2" style="width:20%">
							<input type="text" name="catename" placeholder="科目名称" class="form-control" value="<?php  echo $subject['sname'];?>" />
						</div>
        ...
        <form action="" method="post" class="form-horizontal form" >
            ...
            <div class="table-responsive panel-body">
                <table class="table table-hover">
                    ...
                    <tbody id="level-list">
                    <?php  if(is_array($subject)) { foreach($subject as $row) { ?>
                    <tr>
...
$(document).ready(function() {
	var e_d = 2 ;
	<?php  if(!(IsHasQx($tid_global,1000262,1,$schoolid))) { ?>
		$(".qx_edit").hide();
		e_d = e_d - 1 ;
	<?php  } ?>
	<?php  if(!(IsHasQx($tid_global,1000263,1,$schoolid))) { ?>
		$(".qx_delete").hide();
		e_d = e_d - 1 ;
	<?php  } ?>
	if(e_d == 0){
